...
function HomeScreen(props) {
  ...
  const [Paging, handleListEnd, SetSubStoryLength, SetFetchMore] = usePaging(
    value => {
      LoadingMore(value);
    },
  );
  const [Users, ErrorMessage, SubStoriesLoader, LoadingMore, FirstLoad] =
    useFetchData(
      props,
      value => {
        SetDataProvder(value);
      },
      value => {
        SetSubStoryLength(value);
      },
      value => {
        SetFetchMore(value);
      },
  ...
  return (
    <View style={{flex: 1, backgroundColor: Colors.BackgroundColor}}>
      <Header
        ...
        HeaderText={props.route.params.User}
      />
