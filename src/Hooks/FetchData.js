...
const useFetchData=(props,SetDataProvder,SetSubStoryLength,SetFetchMore)=>{
...
React.useEffect(()=>{
  ...
  React.useEffect(()=>{if(Users.length > 0){SetDataProvder(Users);SetSubStoryLength(Users.length);SetFetchMore(false);}},[Users])
  ...
  async function LoadingMore(Amount){
    ...
    try{
      let UsersArray = await GetUsers({user:props.route.params.User,Following:props.route.params.Type},Amount)
